# сжимает изображения
# работает только с mnist и cifar-10
...
AE_WEIGHTS_FILE = 'ae_compression_weights.weights.h5'
...
class AE(tf.keras.Model):
    def __init__(self, target_compression=0.1, min_latent=64, **kwargs):
        ...
        self.encoder = tf.keras.Sequential([
            ...
            layers.BatchNormalization(),
            layers.Conv2D(128, 3, strides=2, activation='relu', padding='same'),  # 8x8x128
            layers.BatchNormalization(),
            layers.Flatten(),
            layers.Dense(latent_dim)
        ])
        self.latent_layer = layers.Dense(min_latent)
...
# Статистика времени
    ...
    def encode(self, inputs):
        ...
        self.encoding_time.append(time.time() - start_time)
        return latent
    
    def decode(self, latent, target_shape):
        start_time = time.time()
        decoded = self.decoder(latent)
        resized = tf.image.resize(decoded, target_shape)
        self.decoding_time.append(time.time() - start_time)
        return resized
    
    def call(self, inputs):
        h, w = tf.shape(inputs)[1], tf.shape(inputs)[2]
        latent = self.encode(inputs)
        return self.decode(latent, (h, w))
...
# После обучения
            original = x_test[0][np.newaxis, ...]  # Тестовое изображение
            latent = ae.encoder.predict(original)
            print(f"Исходный размер: {original.nbytes} байт")
            print(f"Сжатое представление: {latent.nbytes} байт")
            print(f"Коэффициент сжатия: {original.nbytes/latent.nbytes:.1f}x")

                    # Дополнительная статистика
            print("\nДетальная статистика:")
            print(f"Всего операций кодирования: {len(ae.encoding_time)}")
            print(f"Среднее время кодирования: {np.mean(ae.encoding_time):.4f} сек")
            print(f"Всего операций декодирования: {len(ae.decoding_time)}")
            print(f"Среднее время декодирования: {np.mean(ae.decoding_time):.4f} сек")
